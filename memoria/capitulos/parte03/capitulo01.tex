...
Es importante recalcar la notación usada para describir las reglas de inferencia de \textbf{SumaBinaria}, en concreto, sobre el significado de \textit{compilación maximal}.\index{compilación maximal} Una \textit{compilación maximal}, en este caso, es una subcadena que verifica la expresión regular de $N$ y no puede extenderse en más direcciones. En el caso de la fórmula \texttt{100+11+1001=101}, tenemos cuatro compilaciones maximales de este tipo: \texttt{100}, \texttt{11}, \texttt{1001} y \texttt{101}, en ese orden.
...
\begin{definicion}[Fórmula verdadera]
\index{fórmula!verdadera}Dado un sistema lógico $\mathcal{G}=(\mathcal{S}, \mathcal{I})$ con $\mathcal{S}=(\mathcal{L}, \mathcal{A}, \mathcal{R})$, decimos que la fórmula $\phi \in \mathcal{S}$ es \emph{verdadera} si $\mathcal{I}(\phi)$ está bien definido y es $\mathcal{I}(\phi)=\text{verdadero}$.
\end{definicion}
...
%Un sistema lógico es \emph{consistente} si no puede producir una contradicción, es decir, dada una fórmula $\phi$, no es posible que $\phi$ y $\neg \phi$ sean ambos teoremas. En caso contrario, decimos que es \emph{inconsistente}.
...
\begin{definicion}[Decibilidad de un sistema lógico]\label{def:decidibilidad-sl}
...
\end{adjustwidth}
puede ser traducida en una fórmula $\phi_S$ equivalente en \normalfont{\textbf{Peano}}. (En este caso, con ``equivalente'' nos referimos a que $S$ es verdadera si y solo si $\phi_S$ es verdadera.)
\end{proposicion}
\begin{proof}
...
%Cuando Gödel se propuso a demostrar su célebre resultado de incompletitud \cite{Godel1931}, se dio cuenta que no era suficiente que el sistema formal fuese consistente, pues era posible probar combinaciones infinitas de secuencias que son contradictorias \cite{Kleene1971}. Es por ello por lo que introdujo la $\omega$\emph{-consistencia}.

%Un sistema lógico no consistente es de poca utilidad, pues en él podemos demostrar cualquier cosa. Esto es lo que se conoce como \emph{principio de explosión} (también conocido como \emph{ex falso quodlibet}, ``de lo falso se sigue cualquier cosa'').

%\vspace{8pt}
%\begin{figure}[H]
%\begin{framed}
%El \emph{principio de explosión} funciona como sigue: consideremos dos afirmaciones contradictorias: $\phi$ y $\neg\phi$. Supongamos que ambas afirmaciones son verdaderas. Entonces, podemos demostrar cualquier otra afirmación $\psi$ usando el siguiente argumento:
%\begin{enumerate}
%    \item Sabemos que $\phi$ es cierto (lo asumimos).
%    \item Del mismo modo, sabemos que $\neg\phi=$ es cierto.
%    \item Por tanto, el enunciado $\phi\vee\psi$ debe ser verdadero, pues la primera parte de la disyunción, $\phi$, es verdadera.
%    \item Sin embargo, como $\neg\phi$ es cierto, la primera parte de la disyunción $\phi$ ha de ser falsa, y por lo tanto la segunda parte $\psi$ debe ser verdadera para que $\phi\vee\psi$ sea verdadera. Es decir, $\psi$ es verdadero.
%
%    \vspace*{-8pt}
%\end{enumerate}

%Formalmente, podemos escribir el principio de explosión como:
%$$\phi_1 \wedge \neg\phi_1 \vdash \phi_2$$
%\end{framed}
%\vspace{-8pt}
%\caption{El principio de explosión}
%\label{fig:explosion}
%\end{figure}

%\begin{definicion}[Interpretar el lenguaje de la aritmética]\label{def:lenguaje-aritmetica}
%Sea $\mathcal{G}$ un sistema lógico. Decimos que $\mathcal{G}$ \emph{interpreta el lenguaje de la aritmética} si hay una traducción de las fórmulas de la aritmética a $\mathcal{G}$ de modo que $\mathcal{G}$ puede probar los axiomas principales de la teoría de números.
%\end{definicion}

%\begin{definicion}[$\omega$-consistencia]\label{def:omega-consistencia}
%Sea $\mathcal{S}$ un sistema formal que interpreta el lenguaje de la aritmética, esto es, en el que hay una traducción de las fórmulas de la aritmética al lenguaje de $\mathcal{S}$ de modo que $\mathcal{S}$ sea capaz de probar los axiomas básicos de la teoría de números.

%Sea $x$ una variable y $A(x)$ una fórmula cerrada de $\mathcal{S}$, entonces decimos que $\mathcal{S}$ es $\omega$\emph{-inconsistente} si $\mathcal{S}$ demuestra
%$$A(0), A(1), A(2), ...$$
%pero también existe un cierto número natural $n$ de modo que $A(n)$ no sea un teorema.

%$\mathcal{S}$ es $\omega$\emph{-consistente} si no es $\omega$-inconsistente.
%\end{definicion}

%La $\omega$\emph{-consistencia} es una propiedad sintáctica. Como podíamos anticipar, esta propiedad es una propiedad más fuerte que la consistencia sintáctica. El siguiente resultado se prueba en \cite{Kleene1971}.

%\begin{proposicion}
%Todo sistema formal $\omega$-consistente es sintácticamente consistente.
%\end{proposicion}

%No entraremos en profundidad en el concepto de $\omega$-consistencia, pues queda fuera del alcance de este trabajo. Simplemente, tendremos en cuenta que es una propiedad de los sistemas formales más restrictiva que la consistencia sintáctica.

%Del mismo modo que hemos visto que un sistema $\omega$-consistente es consistente, podemos ver que la solidez es una propiedad más restrictiva que la consistencia semántica.

%\begin{proposicion}
%Todo sistema lógico sólido es semánticamente consistente.
%\end{proposicion}
%\begin{proof}
%Debemos probar que $\phi\wedge\neg\phi$ es falso.
%\end{proof}
\vfill
% ====================
\begin{figure}[H]
