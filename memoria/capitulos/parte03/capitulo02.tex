...
%En la \cref{sec:primera-aproximacion}, probaremos la incompletitud de \textbf{Peano}. Llamaremos a este resultado \emph{Primer Teorema de Incompletitud}. A continuación, generalizaremos este teorema para otros sistemas lógicos. Finalmente, en la \cref{sec:diferencias-godel}, veremos el enunciado original del teorema y lo compararemos con el resultado obtenido.

\section{Una primera aproximación}\label{sec:primera-aproximacion}

%\begin{teorema}[Versión no constructiva del Primer Teorema de Incompletitud]\label{teo:inco1-no-constr}
%La aritmética de Peano es incompleta: existen fórmulas verdaderas de \textbf{Peano} que no son teoremas.
%\end{teorema}
%\begin{proof}
%Asumiremos que \textbf{Peano} es completo y entraremos en contradicción. En concreto, demostraremos que si la aritmética de Peano es completa, podremos decidir \textbf{ParadaEnVacío}, lo cual contradice la \cref{prop:parada-en-vacio-no-decidible}.

%Sea $P$ un programa cualquiera. Lo usaremos como entrada al \cref{lst:parada-en-vacio-desde-peano-completo}. En la \cref{line:parada-en-vacio-desde-peano-completo-import1} importamos la función \texttt{es\_prueba\_peano}, que dada una fórmula y una demostración en \textbf{Peano} comprueba si la demostración prueba que la fórmula es un teorema. Claramente, esta función no es un oráculo. De hecho, su implementación es sencilla: basta probar si cada una de las fórmulas de la demostración es un axioma o se sigue de las fórmulas anteriores a través de las reglas de inferencia del sistema, comprobando que la última fórmula coincida con la fórmula que queremos probar.

%A continuación, importamos en la \cref{line:parada-en-vacio-desde-peano-completo-import2} la función \texttt{parada\_a\_peano} cuya existencia comentamos tras la \cref{prop:parada-a-peano}, que pasa la afirmación ``$P$ para con entrada vacía'' a una fórmula en \textbf{Peano}.

%Usamos esta función en la \cref{line:parada-en-vacio-desde-peano-completo-para-en-peano} para generar la fórmula mencionada anteriormente. En la \cref{line:parada-en-vacio-desde-peano-completo-no-para-en-peano} creamos una nueva fórmula negando la anterior, por tanto \texttt{no\_para\_en\_peano} contiene la fórmula de \textbf{Peano} correspondiente a la afirmación ``$P$ no para con entrada vacía''.

%Una de estas afirmaciones ha de ser verdadera. Como asumimos que \textbf{Peano} es completo, una de estas fórmulas tiene una prueba (es un teorema) en \textbf{Peano}. El bucle de la \cref{line:parada-en-vacio-desde-peano-completo-while} va iterando a través de todos los \emph{strings} posibles, comprobando cada uno para saber si es una prueba de \texttt{para\_en\_peano} o de \texttt{no\_para\_en\_peano}. Ya que uno de ellos debe tener una prueba, el programa terminará, decidiendo si $P$ para con entrada vacía o no. El \cref{lst:parada-en-vacio-desde-peano-completo} resuelve \textsc{ParadaEnVacío}, lo que entra en contradicción con la \cref{prop:parada-en-vacio-no-decidible}.
%\end{proof}

%\begin{lstlisting}[language=Python, caption=\lstinline{parada_en_vacio_desde_peano_completo.py},label={lst:parada-en-vacio-desde-peano-completo}]
%import utilidades

%from es_prueba_peano import es_prueba_peano # NO es un oráculo|\label{line:parada-en-vacio-desde-peano-completo-import1}|
%from parada_a_peano import parada_a_peano # NO es un oráculo|\label{line:parada-en-vacio-desde-peano-completo-import2}|

%def parada_en_vacio_desde_peano_completo(programa):|\label{line:parada-en-vacio-desde-peano-completo-main}|
%    para_en_peano = parada_a_peano(programa)|\label{line:parada-en-vacio-desde-peano-completo-para-en-peano}|
%    no_para_en_peano = 'NOT (' + para_en_peano + ')'|\label{line:parada-en-vacio-desde-peano-completo-no-para-en-peano}|
%    prueba = ''|\label{line:parada-en-vacio-desde-peano-completo-prueba}|
    
%    while True:|\label{line:parada-en-vacio-desde-peano-completo-while}|
%        if es_teorema_peano(prueba, para_en_peano) == 'sí':|\label{line:parada-en-vacio-desde-peano-completo-if1}|
%            return 'sí'
%        if es_teorema_peano(prueba, no_para_en_peano) == 'sí':|\label{line:parada-en-vacio-desde-peano-completo-if2}|
%            return 'no'
%        prueba = utilidades.siguiente_string(prueba)|\label{line:parada-en-vacio-desde-peano-completo-else}|
%\end{lstlisting}
...
\index{primer teorema de incompletitud}\index{teorema de incompletitud}La aritmética de Peano es incompleta: existen fórmulas verdaderas de \normalfont{\textbf{Peano}} que no son teoremas. Concretamente, la fórmula guardada en la variable \texttt{no\_para\_en\_peano} en la \cref{line:godel-peano-formulaneg} del \cref{lst:godel} es un ejemplo de una fórmula verdadera pero no demostrable (no es un teorema) de \normalfont{\textbf{Peano}}.
\end{teorema}
\begin{proof}
...
%\section{El Segundo Teorema de Incompletitud}\label{sec:teo-incompletitud-2}

%\begin{teorema}[Segundo Teorema de Incompletitud]
%La consistencia de la aritmética de Peano no puede probarse en la aritmética de Peano. En concreto, la afirmación
%\begin{adjustwidth}{30pt}{}
%    ``\textbf{Peano} es consistente''
%\end{adjustwidth}
%que puede codificarse mediante una fórmula de \textbf{Peano}, no es un teorema.
%\end{teorema}

Acabamos de demostrar que \textbf{Peano} es incompleto, como ya anticipamos en la \cref{tab:sistemas-logicos}. Las consecuencias de este hecho las exploramos en detalle en el \cref{ch:conclusion}. Sin embargo, paremos un momento a analizar qué es lo que acabamos de probar.
...
% WIP relajar condición de solidez?
...
%Tenemos respuestas a las preguntas (1) y (3) para unas hipótesis concretas pero, ¿qué pasa en el caso de sistemas no recursivamente enumerables y que no son de tipo Turing? Algunos matemáticos no están convencidos de que los resultados de Gödel resuelvan definitivamente el segundo problema de Hilbert, pues los teoremas de Gödel no se aplican en otros sistemas de lógica.
...
%- Autores incluyendo J. R. Lucas y Roger Penrose debaten que los teoremas tienen implicaciones sobre la inteligencia humana. El debate se centra sobre si la mente humana es equivalente a una máquina de Turing o (mediante la tesis de Church-Turing) a una máquina finita. Si la mente lo es, y los teoremas de Gödel se aplican a esta, los teoremas de incompletitud se aplicarían a ella.
...
%Consecuencias severas para el logicismo de Gottlob Frege y Bertrand Russell, que pretendían definir los números naturales en términos de lógica
...
%\begin{tabular}{@{}ll@{}}
%\toprule
%Máquinas de Turing & Numeración Gödel \\
%(\cref{teo:incompletitud-semantica}) & (\cref{teo:incompletitud-godel}) \\ \midrule
%para sistemas lógicos (semántica) & para sistemas formales (sintaxis) \\[4pt]
%asume que el sistema lógico es sólido & asume que el sistema formal es $\omega$-consistente \\[4pt]
%usa sistemas lógicos capaces de probar & usa sistemas formales capaces de probar \\
%afirmaciones del tipo ``$P$ para con &  ciertos teoremas de aritmética elemental \\
%entrada vacía'' \\ \bottomrule
%\end{tabular}
%\end{adjustwidth}
%\end{table}
%\vspace{-8pt}
%\caption{Diferencias en los teoremas de incompletitud mostrados en este capítulo}
%\label{tab:diferencias-turing-godel}
%\end{tabla}
...
%\begin{lema}\label{lema:red-prob}
%Sea la siguiente fórmula un teorema:
%$$\phi_1\Rightarrow\phi_2$$

%Entonces, si $\phi_2$ no es un teorema, $\phi_1$ tampoco lo es.
%\end{lema}
%\begin{proof}
%Este resultado se obtiene de la implicación inmediata siguiente: si $\phi_1$ es un teorema, entonces $\phi_2$ ha de serlo también, pues se deduce inmediatamente de $\phi_1\Rightarrow\phi_2$ mediante \emph{modus ponens}. Concluimos la prueba por el contrarrecíproco.
%\end{proof}

%\begin{teorema}[Versión semántica del Segundo Teorema de Incompletitud]
%Sea $\mathcal{G}$ un sistema lógico que verifique las siguientes condiciones:
%\begin{itemize}
%    \item[(A2)] $\mathcal{G}$ es recursivamente axiomatizable.
%    \item[(A3)] $\mathcal{G}$ puede expresar afirmaciones sobre máquinas de Turing o, equivalentemente, sobre programas; en concreto, dada una máquina de Turing $M$, puede expresar que ``$M$ para con entrada vacía'' o, equivalentemente, dado un programa $P$, puede expresar que ``$P$ para con entrada vacía''.
%\end{itemize}
%Entonces, la fórmula correspondiente a ``$\mathcal{G}$ es sólido'' no es un teorema en $\mathcal{G}$.
%\end{teorema}
%\begin{proof}
%El \cref{teo:incompletitud-semantica} nos dice que, suponiendo (A2) y (A3), en caso de que $\mathcal{G}$ fuese sólido, entonces tendríamos una fórmula $\phi$ que (1) es verdadera pero (2) no es un teorema.

%Como hemos logrado probar (1) en $\mathcal{G}$, tenemos el siguiente teorema:
%$$``\mathcal{G}\text{ es sólido}''\Rightarrow\phi$$
%Ahora bien, aplicando (2) y el \cref{lema:red-prob}, tenemos que ``$\mathcal{G}$ es sólido'' no es un teorema.
%\end{proof}

%\emph{\textbf{Nota: 6.1} para probar la versión sintáctica (que es el verdadero segundo teorema de incompletitud) sería necesario encontrar una versión constructiva del \cref{teo:incompletitud-sintactica}, que nos diese una fórmula $\phi$ que fuese verdadera de modo que ni $\phi$ ni $\neg\phi$ sean teoremas. Pero esto es un tanto extraño porque si el teorema es sintáctico, ¿qué quiere decir que una fórmula sea verdadera?}

%\section{Consecuencias}\label{sec:teo-incompletitud-consecuencias}
